...
import uniandes.dpoo.aerolinea.modelo.tarifas.CalcularTarifasTemporadaAlta;
import uniandes.dpoo.aerolinea.modelo.tarifas.CalcularTarifasTemporadaBaja;
...
public class Aerolinea
{
    ...
    {
        try {IPersistenciaTiquetes cargador = CentralPersistencia.getPersistenciaTiquetes( tipoArchivo );
    ...
    {
    	boolean avionOcupado = false;
    	
    	try {
        for (Vuelo vuelo: vuelos) {
        	String avion = vuelo.getAvion().getNombre();
        	String fechaVuelo = vuelo.getFecha();
        	
			if (avion.equals(nombreAvion) && fechaVuelo.equals(fecha)) {
				avionOcupado = true;
			}			
        }
		...
		if (!avionOcupado) {
			...
			new Vuelo(rutas.get(codigoRuta), fecha, avionVuelo);
		}}
     ...
     */
    public double venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	Vuelo vuelo = getVuelo(codigoRuta, fecha);
    	Cliente cliente = getCliente(identificadorCliente); 
    	
		if (( vuelo.getAvion().getCapacidad() < (vuelo.getTiquetes()).size() +cantidad)) {
		...
		try {
		String mes = vuelo.getFecha().substring(5,7);
		int mesNumero = Integer.parseInt(mes);
		
		if (!(mesNumero <= 5 || mesNumero>=9 && mesNumero <= 11))
		{
    	CalcularTarifasTemporadaBaja calculadora = new CalcularTarifasTemporadaBaja();
        return vuelo.venderTiqutes(cliente, calculadora, cantidad);
	} 
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
		...
		List<Tiquete>tiquetes = cliente.getTiquetesSinUsar();
